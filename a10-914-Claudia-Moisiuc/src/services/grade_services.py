...
class GradeService:
    ...
    def random_data(self, student_repository, assignment_repository):
        ...
        self.__repository.filter_note()

    def availability(self, id):
        """The function verifies if an id is available. """
        n = len(self.__repository.database)
        for ids in range(0, n):
            if id == self.__repository.database[ids]:
                return False
        return True
    ...
    def existence(self, sid):
        ...
        n = len(self.__repository.database)
        for ids in range(0, n):
            if sid == self.__repository.database[ids].student_id:
                return True
        return False
    ...
    def graded(self, sid, aid):
        ...
        n = len(self.__repository.database)
        for ids in range(0, n):
            if sid == self.__repository.database[ids].student_id and aid == self.__repository.database[ids].assign_id:
                if self.__repository.database[ids].grade is not None:
    ...
    def give_grade(self, sid, aid, gr=None):
        ...
        n = len(self.__repository.database)
        for i in range(0, n):
            if sid == self.__repository.database[i].student_id and aid == self.__repository.database[i].assign_id:
                g = Grade(aid, sid, gr)
                self.__repository.__setitem__(i, g)
    ...
    def remove(self, id):
        ...
        ids = 0
        while ids < n:
            if id == self.__repository.database[ids].assign_id:
                self.__repository.__delitem__(ids)
                n = n - 1
            else:
                ids = ids + 1
    ...
    def removes(self, id):
        ...
        ids = 0
        while ids < n:
            if id == self.__repository.database[ids].student_id:
                self.__repository.__delitem__(ids)
                n = n - 1
            else:
                ids = ids + 1
    ...
    def ad(self, l, ida):
        n = len(l)
        for i in range(0, n):
            self.__repository.__add__(ida, l[i])

    def remo(self, ids, ida):
        n = len(self.__repository.database)
        idd = 0
        while idd < n:
            if ids == self.__repository.database[idd].student_id and ida == self.__repository.database[idd].assign_id:
