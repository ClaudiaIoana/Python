...
class StatisticService:
    ...
    def list_order(self, assi):
        ...
        for ids in range(0, n):
            if assi == self.__grade_service.database[ids].assign_id and self.__grade_service.database[ids].grade is not None:
    ...
    def list_assi(self):
        ...
        n = len(self.__grade_service.database)
        for i in range(0, n):
            if self.__grade_service.database[i].grade is None and self.unique(self.__grade_service.database[i].student_id, li) is True:
                data = date.today()
